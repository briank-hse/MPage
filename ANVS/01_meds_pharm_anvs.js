...
function setActiveNav(idx) {
  for (var i = 1; i <= totalBlobs; i++) {
    ...
    var base = n.getAttribute('data-basecls') || 'gp-nav-item';
    n.className = base + (i === idx ? ' active-nav' : '');
  }
...
function clearCompareHighlights() {
  for (var i=1;i<=totalBlobs;i++){var n=document.getElementById('nav-'+i); if(n){n.className=n.getAttribute('data-basecls')||'gp-nav-item';}}
}
...
function handleCompareClick(idx) {
  ...
  if (n) n.className = (n.getAttribute('data-basecls')||'gp-nav-item') + ' compare-sel';
  var banner = document.getElementById('compare-banner');
...
function buildSidebarFlags() {
  for (var i=1;i<=totalBlobs;i++){
    ...
    for(var h=0;h<hits.length;h++){var sp=document.createElement('span');sp.className='nav-flag nav-flag-highrisk';sp.textContent=hits[h];flags.appendChild(sp);}
    nav.appendChild(flags);
  }
  for(var i=1;i<=totalBlobs;i++){var n=document.getElementById('nav-'+i);if(n&&!n.getAttribute('data-basecls'))n.setAttribute('data-basecls',n.className);}
}
