...
HEAD REPORT
    ...
    ROW + 1 call print(^  var views = ['scroll-side', 'scroll-main', 'gp-table', 'med-container', 'dot-view', 'acuity-view'];^)
    ...
    ROW + 1 call print(^  var views = ['med-list', 'header-row-inf', 'gp-blob-view', 'med-container', 'dot-view', 'acuity-view'];^)
    ROW + 1 call print(^  for (var i = 0; i < views.length; i++) {^)
    ROW + 1 call print(^    var el = document.getElementById(views[i]);^)
    ROW + 1 call print(^    if (el) el.style.display = 'none';^)
    ROW + 1 call print(^  }^)
    ROW + 1 call print(^  for (var j = 1; j <= 6; j++) {^)
