...
IF (CNVTREAL($WARD_CD) > 0.0)
    ...
    DETAIL
      call print(CONCAT(
          ...
          "<td><a class='patient-link' href='javascript:APPLINK(1,",
          CHAR(34), "Powerchart.exe", CHAR(34), ",",
          CHAR(34), "/PERSONID=", TRIM(CNVTSTRING(rec_cohort->list[D.SEQ].person_id)),
          " /ENCNTRID=", TRIM(CNVTSTRING(rec_cohort->list[D.SEQ].encntr_id)),
          " /FIRSTTAB=^Pharmacist MPage^", CHAR(34), ")'>",
