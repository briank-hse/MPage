...
IF (CNVTREAL($WARD_CD) > 0.0)
    ...
    DECLARE v_ward_rows = c32767 WITH noconstant("")
    DECLARE v_summary_rows = c32767 WITH noconstant("")
    DECLARE v_matrix_rows = c32767 WITH noconstant("")
    DECLARE pat_idx = i4 WITH noconstant(0)
    DECLARE idx = i4 WITH noconstant(0)
    DECLARE t_score = i4 WITH noconstant(0)
    DECLARE t_triggers = c32767 WITH noconstant("")
    ...
    IF (num_pats > 0)
            ...
            IF (idx > 0)
                IF (CE.EVENT_CD = 15071366.00) 
                    ...
                    IF (rec_cohort->list[idx].det_transfusion > "")
                        rec_cohort->list[idx].det_transfusion = CONCAT(rec_cohort->list[idx].det_transfusion, ", ", TRIM(TITLE, 3), ": ", TRIM(VAL, 3), " mL")
                    ELSE
                        rec_cohort->list[idx].det_transfusion = CONCAT(TRIM(TITLE, 3), ": ", TRIM(VAL, 3), " mL")
                    ENDIF
                ELSEIF (CE.EVENT_CD IN (82546829.00, 15083551.00, 19995695.00) AND CNVTREAL(CE.RESULT_VAL) > 1000.0) 
                    ...
                    IF (rec_cohort->list[idx].det_ebl > "")
                        rec_cohort->list[idx].det_ebl = CONCAT(rec_cohort->list[idx].det_ebl, ", ", TRIM(TITLE, 3), ": ", TRIM(VAL, 3), " mL")
                    ELSE
        ...
        FOR (pat_idx = 1 TO num_pats)
            ...
            IF (rec_cohort->list[pat_idx].flag_transfusion = 1 OR rec_cohort->list[pat_idx].flag_ebl = 1) SET t_score = t_score + 3 SET t_triggers = CONCAT(t_triggers, "<span class='trig-pill' title='", TRIM(CONCAT(TRIM(rec_cohort->list[pat_idx].det_transfusion, 3), " ", TRIM(rec_cohort->list[pat_idx].det_ebl, 3)), 3), "'>Haemorrhage/Transfusion</span>") ENDIF
