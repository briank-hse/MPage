 ...
 * tab-dev3  : Antimicrobial DOT Edge 4 (ghost) (01_meds_dot_date_comb_edge4)
 *
 * NICU PRINT LABELS:
...
declare v_xcr_prog_dot4   = vc with noconstant("01_meds_dot_date_comb_edge4:Group1")
...
declare v_js_prog_dot4     = vc with noconstant("")
...
set v_js_prog_dot4     = build2("var PROG_DOT4     = '", v_xcr_prog_dot4, "';")
...
detail
  ...
  row +1 v_js_prog_dot4
  row +1 v_js_prog_nicu
  ...
  row +1 "  loadTab(PROG_DOT, PARAMS_DOT + ',' + Date.now(), 'tab-dot');"
  row +1 "}"
  row +1 ""
  row +1 "function loadDOTEdge4() {"
  row +1 "  loadTab(PROG_DOT4, PARAMS_DOT + ',' + Date.now(), 'tab-dev3');"
  row +1 "}"
  row +1 ""
  row +1 "function loadNICU() {"
  row +1 "  loadTab(PROG_NICU, PARAMS_NICU + ',' + Date.now(), 'tab-nicu');"
  row +1 "}"
  row +1 ""
  ...
  row +1 ~  <button id='tab-dev3' class='tab-btn ghost' onclick='loadDOTEdge4()'>&nbsp;</button>~
  row +1 "</div>"
  ...
  ; that contains inline <script> tags and CCLLINK calls.
