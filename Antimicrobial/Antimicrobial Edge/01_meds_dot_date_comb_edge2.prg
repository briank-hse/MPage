...
/*****************************************************************************
  ...
  - EDGE REWRITE: Replaced flexbox '.strip' arrays with pure <td> grids for perfect alignment.
...
declare v_colgroup_html = vc with noconstant(""), maxlen=65534
...
/* EDGE REWRITE: Replaced <span> flex elements with native <th> table elements */
if (v_first = 1)
  set v_days = 0
  set v_axis_html = ""
  ...
  while (v_i < v_days)
    if (v_i = 0 or format(v_min_dt + v_i,"MM;;D") != format(v_min_dt + v_i - 1,"MM;;D"))
      set v_header_html = concat(v_header_html,
        '<th class="tick" title="', format(v_min_dt + v_i,"YYYY-MM-DD;;D"), '">',
          '<span class="mo">', format(v_min_dt + v_i,"MMM;;D"), '</span>',
          format(v_min_dt + v_i,"DD;;D"), '</th>'
      )
    else
      set v_header_html = concat(v_header_html,
        '<th class="tick" title="', format(v_min_dt + v_i,"YYYY-MM-DD;;D"), '">',
  ...
  /* Build explicit <colgroup> so day columns stay fixed-width in Edge */
...
foot med_name
  if (v_days > 0)
    ...
    /* EDGE REWRITE: generating raw <td> tags per day */
    while (v_i < v_days)
      ...
      if (v_count_i > 0)
        ...
        v_strip = concat(v_strip, '<td class="cell on" title="', v_title, '" data-count="', trim(v_count_str), '">', trim(v_count_str), '</td>')
      else
        v_strip = concat(v_strip, '<td class="cell" title="', format(v_min_dt + v_i,"DD/MM/YYYY;;D"), '"></td>')
      endif
      v_i = v_i + 1
    endwhile

    /* EDGE REWRITE: Native grid integration. No nested <div> wrapper. */
...
foot report
  if (v_days > 0)
      ...
      v_spacer_strip = ""
      v_i = 0
      /* EDGE REWRITE: <td> natively generated */
      while (v_i < v_days)
          v_key8 = format(v_min_dt + v_i, "YYYYMMDD;;D")
          if (findstring(concat("~", v_key8, "~"), v_all_days_list) > 0)
             v_sum_strip = concat(v_sum_strip, '<td class="cell sum-yes" title="Antimicrobial Administered"></td>')
          else
              v_sum_strip = concat(v_sum_strip, '<td class="cell sum-no" title="No Antimicrobials"></td>')
          endif

          v_spacer_strip = concat(v_spacer_strip, '<td class="cell spacer-bit"></td>')
          v_i = v_i + 1
      endwhile
  ...
  if (v_max_track > 0)
    for (v_t = 1 to v_max_track)
      ...
      while (v_i < v_days)
        ...
        /* EDGE REWRITE: Generate raw <td> for Encounters row */
        set v_strip = concat(v_strip, '<td class="', v_cell_class, '" title="', v_cell_title, '"><span class="enc-cell-text">', v_cell_text, '</span></td>')
        set v_i = v_i + 1
      endwhile
...
head report
  ...
  row +1 '<style>'
  row +1 ':root{--bg-main:#fff;--bg-alt:#f5f5f5;--border-color:#d6d9dd;--border-dark:#b5b5b5;--cerner-blue:#0086CE;--w-med:150px;--w-dose:32px;--w-dot:32px;'
  ...
  /* Explicit Table Column Definitions */
  row +1 'table.chart-tbl th.sticky-med, table.chart-tbl td.sticky-med { position:sticky; left:0; background:var(--sticky-bg); z-index:10; border-right:1px solid var(--border-dark); border-bottom:1px solid var(--border-color); padding-left:8px; width:var(--w-med); min-width:var(--w-med); max-width:var(--w-med); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }'
  row +1 'table.chart-tbl th.sticky-doses, table.chart-tbl td.sticky-doses { position:sticky; left:var(--w-med); background:var(--sticky-bg); z-index:10; border-right:1px solid var(--border-dark); border-bottom:1px solid var(--border-color); width:var(--w-dose); min-width:var(--w-dose); max-width:var(--w-dose); text-align:center; }'
  row +1 'table.chart-tbl th.sticky-dot, table.chart-tbl td.sticky-dot { position:sticky; left:calc(var(--w-med) + var(--w-dose)); background:var(--sticky-bg); z-index:10; border-right:none; box-shadow: 2px 0 5px -2px rgba(0,0,0,0.2); border-bottom:1px solid var(--border-color); width:var(--w-dot); min-width:var(--w-dot); max-width:var(--w-dot); text-align:center; }'
  
  /* Month Labels Layer Override */
  row +1 'table.chart-tbl thead tr.ticks th.sticky-med, table.chart-tbl thead tr.ticks th.sticky-doses, table.chart-tbl thead tr.ticks th.sticky-dot {background: var(--bg-main) !important; z-index: 50 !important; border-bottom:1px solid var(--border-dark); clip-path: inset(0); transform: translateZ(0); }'
  ...
  /* Equalize the box models perfectly */
  row +1 'table.chart-tbl th.sticky-med, table.chart-tbl td.sticky-med, table.data-tbl th:nth-child(1), table.data-tbl td:nth-child(1) { box-sizing: border-box !important; }'
  row +1 'table.chart-tbl th.sticky-doses, table.chart-tbl td.sticky-doses, table.data-tbl th:nth-child(2), table.data-tbl td:nth-child(2) { box-sizing: border-box !important; }'
  row +1 'table.chart-tbl th.sticky-dot, table.chart-tbl td.sticky-dot, table.data-tbl th:nth-child(3), table.data-tbl td:nth-child(3) { box-sizing: border-box !important; }'
  ...
  row +1 '.cell { border:1px solid #ccc; background:var(--bg-main); border-radius:2px; }'
  row +1 '.cell.on { background:var(--cerner-blue); border-color:#0D66A1; color:var(--bg-main); font-weight:600; font-size:10px; }'
  ...
  row +1 '<table class="chart-tbl">'
  row +1 call print(v_colgroup_html)
  row +1 '<thead>'
  
  /* EDGE REWRITE: Table-native axis generation */
  row +1 call print(concat('<tr><th class="label sticky-med">Medication</th><th class="label sticky-doses">Doses</th><th class="label sticky-dot">DOT</th><th class="label axis-header" colspan="', trim(cnvtstring(v_days), 3), '">Days'))
  row +1 call print(v_axis_html)
  row +1 '</th></tr>'
  if (textlen(v_header_html) > 0)
    row +1 '<tr class="ticks"><th class="sticky-med"></th><th class="sticky-doses"></th><th class="sticky-dot"></th>'
